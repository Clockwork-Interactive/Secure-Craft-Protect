package net.zeus.scppancakes.custom;
...
public class RunnableCooldownHandler { // Thank you, Connor!
    ...
    public static void serverTick(TickEvent.ServerTickEvent event) {
        for (Map.Entry<Runnable, Integer> entry : RUNNABLE_TICK_MAP.entrySet()) {
            if (entry.getValue() == 0) {
                try {
                    entry.getKey().run();
                } catch (Exception e) {
                ...
                RUNNABLE_TICK_MAP.remove(entry.getKey());
                continue;
            }

            RUNNABLE_TICK_MAP.replace(entry.getKey(), entry.getValue() - 1);
        }
    }
